...
static ssize_t current_speed_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
	pr_buf_info("@@@@%s:%d\n", __func__, current_speed);
	return sprintf(buf,"%s\n",usb_speed_string(current_speed));
}
USBFACTORY_ATTR(current_speed);
static int current_speed_debugfs_init(struct usb_device *hdev)
{
	static bool node_created = false;
	int ret = 0;

	if(!node_created) {
		ret = his_register_sysfs_attr(&current_speed_attr.attr);
		if(ret < 0)
			pr_err("%s,Couldn't create current_speed file ret = %d\n",__func__,ret);
		else
			node_created = true;
	}
...
static int hub_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	current_speed_debugfs_init(hdev);

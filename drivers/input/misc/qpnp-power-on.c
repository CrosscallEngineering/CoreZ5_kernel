...
#define QPNP_PON_KPDPWR_S1_TIMER(pon)		((pon)->pbs_base + 0x40)
#define QPNP_PON_KPDPWR_S2_TIMER(pon)		((pon)->pbs_base + 0x41)
#define QPNP_PON_KPDPWR_S2_CNTL(pon)		((pon)->pbs_base + 0x42)
#define QPNP_PON_KPDPWR_S2_CNTL2(pon)		((pon)->pbs_base + 0x43)
#define QPNP_PON_RESIN_S1_TIMER(pon)		((pon)->pbs_base + 0x44)
#define QPNP_PON_RESIN_S2_TIMER(pon)		((pon)->pbs_base + 0x45)
#define QPNP_PON_RESIN_S2_CNTL(pon)		((pon)->pbs_base + 0x46)
#define QPNP_PON_RESIN_S2_CNTL2(pon)		((pon)->pbs_base + 0x47)
#define QPNP_PON_KPDPWR_RESIN_S1_TIMER(pon)	((pon)->pbs_base + 0x48)
#define QPNP_PON_KPDPWR_RESIN_S2_TIMER(pon)	((pon)->pbs_base + 0x49)
#define QPNP_PON_KPDPWR_RESIN_S2_CNTL(pon)	((pon)->pbs_base + 0x4A)
#define QPNP_PON_KPDPWR_RESIN_S2_CNTL2(pon)	((pon)->pbs_base + 0x4B)
...
static struct qpnp_pon *primary_pon_dev;
...
static void longkey_fs_sync_func(struct work_struct *work)
{
	struct qpnp_pon *pon =
			container_of(work, struct qpnp_pon, fsync_timer.work);
	int rc;
	u32 key_status;
	uint pon_rt_sts;
	...
	key_status = (pon_rt_sts & QPNP_PON_GEN3_KPDPWR_N_SET);
	if (!key_status) {
		dev_err(pon->dev, "Key is release, Cancel the work\n");
		return;
	}

	dev_err(pon->dev, "Found the Long press power key, run fs sync\n");
	ksys_sync();
}
...
{
	int rc = 0;
	struct qpnp_pon *pon = primary_pon_dev;
	struct qpnp_pon_config *cfg = NULL;
...
void hs_set_pm_pon_resin(int debug)
{
	...
	if (NULL == primary_pon_dev) {
		WARN(1, "%s: primary_pon_dev is NULL\n", __func__);
		return;
	}
...
static int qpnp_pon_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&pon->fsync_timer, longkey_fs_sync_func);

	if(to_spmi_device(dev->parent)->usid == 0)
		primary_pon_dev = pon;


	pr_err("%s, sys_reset=%d, dev_name=%s\n", __func__, sys_reset, pon->dev->kobj.name);
	if (sys_reset)
		sys_reset_dev = pon;

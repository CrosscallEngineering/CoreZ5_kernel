...
enum battery_property_id {
	...
	BATT_PARALLEL_CHARGING_ENABLE,
	BATT_FAST_CHARGE_CURRENT,
	BATT_PROP_MAX,
};
...
static const int battery_prop_map[BATT_PROP_MAX] = {
	...
	[BATT_CHARGING_ENABLE]	= POWER_SUPPLY_PROP_CHARGING_ENABLED,
	[BATT_PARALLEL_CHARGING_ENABLE]	= POWER_SUPPLY_PROP_PARALLEL_CHARGING_ENABLED,
	[BATT_FAST_CHARGE_CURRENT]	= POWER_SUPPLY_PROP_FAST_CHARGE_CURRENT,
};
...
static const int usb_prop_map[USB_PROP_MAX] = {
	...
	[USB_CC_ORIENTATION]		= POWER_SUPPLY_PROP_CC_ORIENTATION,
	[USB_PRESENT]		= POWER_SUPPLY_PROP_PRESENT,
	[OTG_PRESENT]		= POWER_SUPPLY_PROP_USB_OTG,
	[BATT_EMPTY]	= POWER_SUPPLY_PROP_BATTERY_EMPTY,
	[BATT_SAFETY_TIMER_ENABLE]	= POWER_SUPPLY_PROP_SAFETY_TIMER_ENABLE,
};
...
static int usb_psy_set_safety_timer_enable(struct battery_chg_dev *bcdev,
					int val)
{
	int rc = 0;
		...
		const union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_SAFETY_TIMER_ENABLE:
		rc = usb_psy_set_safety_timer_enable(bcdev, pval->intval);
		break;
	default:
		break;
	}
...
static enum power_supply_property usb_props[] = {
	...
	POWER_SUPPLY_PROP_CC_ORIENTATION,
	POWER_SUPPLY_PROP_PRESENT,
	POWER_SUPPLY_PROP_USB_OTG,
	POWER_SUPPLY_PROP_BATTERY_EMPTY,
...
static int battery_psy_set_charging_enabled(struct battery_chg_dev *bcdev,
					int val)
{
	int rc = 0;

	rc = write_property_id(bcdev, &bcdev->psy_list[PSY_TYPE_BATTERY],
				BATT_CHARGING_ENABLE, val);
		if (rc < 0) {
			pr_err("Failed to set val %d, rc=%d\n", val, rc);
		} else {
			pr_err("Set val to %d \n", val);
...
static int battery_psy_set_parallel_charging_enabled(struct battery_chg_dev *bcdev,
					int val)
{
	int rc = 0;

	rc = write_property_id(bcdev, &bcdev->psy_list[PSY_TYPE_BATTERY],
				BATT_PARALLEL_CHARGING_ENABLE, val);
		if (rc < 0) {
			pr_err("Failed to set val %d, rc=%d\n", val, rc);
		} else {
			pr_err("Set val to %d \n", val);
...
static int battery_psy_set_fast_charge_current(struct battery_chg_dev *bcdev,
					int val)
{
	int rc = 0;
		...
		union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		if (sgm_mag_charger_ok()) {
			...
			power_supply_get_property(bcdev->sgm_chg_psy, POWER_SUPPLY_PROP_STATUS, pval);
		}else
			pval->intval = pst->prop[prop_id];
		...
		const union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		return battery_psy_set_charging_enabled(bcdev, pval->intval);
	case POWER_SUPPLY_PROP_PARALLEL_CHARGING_ENABLED:
		return battery_psy_set_parallel_charging_enabled(bcdev, pval->intval);
	case POWER_SUPPLY_PROP_FAST_CHARGE_CURRENT:
		return battery_psy_set_fast_charge_current(bcdev, pval->intval);
	default:
		return -EINVAL;
	}
...
static enum power_supply_property battery_props[] = {
	...
	POWER_SUPPLY_PROP_PARALLEL_CHARGING_ENABLED,
...
static void qti_chg_status_update_work(struct work_struct *work)
{
	...
	struct charge_status qti_charge_status = {0,0,0,0,0,0,0,0};
	struct psy_state *pst = &bcdev->psy_list[PSY_TYPE_USB];
	...
	ret = power_supply_get_property(usb_psy, POWER_SUPPLY_PROP_BATTERY_EMPTY,&pval);
	if (ret) {
		dev_err(bcdev->dev, "get failed to get battery empty now %d\n", ret);
		return;
	}
	qti_charge_status.vbat_empty = pval.intval;

	if (qti_charge_status.vbat_empty) {
		if (!dead_wake_flag) {
			dead_wake_flag = true;
	...
	ret = read_property_id(bcdev, pst, USB_REAL_TYPE);
	if (ret) {
		dev_err(bcdev->dev, "get failed to get usb real type %d\n", ret);
		return;
	}

	pr_err("hs_info :: vbus=%d, usb_online=%d, ibatt_now=%d, vbatt_now=%d, capacity=%d, temp=%d, input_current_now=%d,real_charger_type=%s\n",
			qti_charge_status.input_voltage_now, qti_charge_status.usb_online, qti_charge_status.ibatt_now, qti_charge_status.vbatt_now,
			qti_charge_status.msoc, qti_charge_status.batt_temp, qti_charge_status.input_current_now,get_usb_type_name(pst->prop[USB_REAL_TYPE]));
...
extern void sgm41512_determine_initial_status(void);
extern void sgm_hw_init(void);
static int battery_chg_probe(struct platform_device *pdev)
{
	...
	sgm_hw_init();
	sgm41512_determine_initial_status();
	return 0;
error:

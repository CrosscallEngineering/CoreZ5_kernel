...
int nfcc_hw_check(struct nfc_dev *nfc_dev)
{
	...
	nci_init_cmd = kzalloc(NCI_INIT_CMD_LEN + 1,
					GFP_DMA | GFP_KERNEL);
	if (!nci_init_cmd) {
		ret = -ENOMEM;
		goto done;
	}
	nci_init_rsp = kzalloc(NCI_INIT_RSP_LEN + 1,
					GFP_DMA | GFP_KERNEL);
	if (!nci_init_rsp) {
		ret = -ENOMEM;
		goto done;
	...
	pr_info("%s: - NFC send init cmd \n", __func__);
	ret = nfc_dev->nfc_write(nfc_dev, nci_init_cmd,NCI_INIT_CMD_LEN, MAX_RETRY_COUNT);
	if (ret <= 0) {
		pr_err("%s: - nfc send init cmd err %d\n",
					__func__, ret);
	}   
	if (nfc_dev->interface == PLATFORM_IF_I2C) {
		ret = is_data_available_for_read(nfc_dev);
		if (ret <= 0) {
			pr_err("%s: - error waiting for init rsp %d\n",
					__func__, ret);
			nfc_dev->nfc_disable_intr(nfc_dev);
			goto err_nfcc_hw_check;
		}
	...
	/* Read Notification of RESET command */
	pr_info("%s: - NFC read init rsp \n", __func__);
	ret = nfc_dev->nfc_read(nfc_dev, nci_init_rsp, NCI_INIT_RSP_LEN);
	if (ret <= 0) {
		pr_err("%s: nfc nfc read init rsp error %d\n", __func__, ret);
		goto err_nfcc_hw_check;
	}
	init_rsp_len = 2 + nci_init_rsp[2];
	if (init_rsp_len > NCI_HDR_LEN) {
		nfc_dev->nqx_info.info.chip_type =
			nci_init_rsp[init_rsp_len - NFC_CHIP_TYPE_OFF];
		nfc_dev->nqx_info.info.rom_version =
			nci_init_rsp[init_rsp_len - NFC_ROM_VERSION_OFF];
		nfc_dev->nqx_info.info.fw_major =
			nci_init_rsp[init_rsp_len - NFC_FW_MAJOR_OFF];
		nfc_dev->nqx_info.info.fw_minor =
